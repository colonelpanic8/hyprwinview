...
static std::vector<CHyprSignalListener> callbackListeners;
...
static SDispatchResult dispatch(std::string arg) {
    ...
    Log::logger->log(LOG, "[Hyprtasking] passthrough dispatch: {} : {}{}", DISPATCHSTR, DISPATCHARG,
...
static SDispatchResult dispatch_if(std::string arg, bool is_active) {
    ...
    if (cursor_view->active != is_active) {
        switch (Config::mgr()->type()) {
            ...
            case Config::CONFIG_LUA: return {};
            case Config::CONFIG_LEGACY:
                return {.passEvent = true, .success = false, .error = "predicate not met"};
                ;
            default: return {.success = false, .error = "what config did vaxry cook again??"};
        }
    }
    return dispatch(arg);
}
...
static void set_layer(PHTVIEW view, int new_layer) {
    ...
    if (view->closing)
        return;
    Log::logger->log(LOG, "[Hyprtasking] View \"{}\", previous layer: {}, new: {}",
                     view->get_monitor()->m_name, view->layout->layer, new_layer);
...
static SDispatchResult change_layer(std::string arg, bool move_window) {
    ...
    const WORKSPACEID target_ws_id = grid->slot_workspace(resulting_layer, src_slot.x, src_slot.y);
...
static void hook_render_workspace(void* thisptr, PHLMONITOR monitor, PHLWORKSPACE workspace,
                                  const Time::steady_tp& now, const CBox& geometry) {
    if (ht_manager == nullptr) {
        ((render_workspace_t)(render_workspace_hook->m_original))(thisptr, monitor, workspace, now,
                                                                  geometry);
        return;
    }
...
// True only between the explicit context markers emitted around our scaled window or
// workspace pass elements. Native renderModif paths -- workspace slides, special
// workspaces, etc. -- remain on Hyprland's normal rendering path.
static bool overview_scaled_render_active() {
    return overview_scaled_render && render_modif_scaled();
}
...
static void hook_render_texture(void* thisptr, SP<Render::ITexture> tex, const CBox& box,
    ...
    if (!overview_scaled_render_active() || render_data.pMonitor == nullptr) {
        ((render_texture_t)(render_texture_hook->m_original))(thisptr, tex, box, data);
        return;
    }

    CRegion full_damage;
    full_damage     = CBox{0, 0, render_data.pMonitor->m_transformedSize.x,
                       render_data.pMonitor->m_transformedSize.y};
    data.damage     = &full_damage;
    data.clipRegion = {};
    data.a *= overview_scaled_alpha;
    const CBox saved_clip_box = render_data.clipBox;
    render_data.clipBox       = overview_scaled_clip.value_or(CBox{});
...
typedef void (*render_border_t)(void* thisptr, const CBox& box,
                                const Config::CGradientValueData&              grad,
                                Render::GL::CHyprOpenGLImpl::SBorderRenderData data);
typedef void (*render_border2_t)(void* thisptr, const CBox& box,
...
static void render_border_scaled(CBox& box, Render::GL::CHyprOpenGLImpl::SBorderRenderData& data,
                                 Fn&& call_original) {
    auto& render_modif = g_pHyprRenderer->m_renderData.renderModif;
    if (!overview_scaled_render_active()) {
        call_original();
        return;
    }
...
static void hook_render_border(void* thisptr, const CBox& box,
                               const Config::CGradientValueData&              grad,
                               Render::GL::CHyprOpenGLImpl::SBorderRenderData data) {
    CBox tbox = box;
...
static void hook_render_border2(void* thisptr, const CBox& box,
                                const Config::CGradientValueData& grad1,
                                const Config::CGradientValueData& grad2, float lerp,
                                Render::GL::CHyprOpenGLImpl::SBorderRenderData data) {
    CBox tbox = box;
    render_border_scaled(tbox, data, [&] {
        ((render_border2_t)(render_border2_hook->m_original))(thisptr, tbox, grad1, grad2, lerp,
                                                              data);
    });
}
...
static void register_monitors() {
    ...
    for (const PHLMONITOR& monitor : State::monitorState()->monitors()) {
        ...
        Log::logger->log(LOG, "[Hyprtasking] Registering view for monitor {} with resolution {}x{}",
                         monitor->m_description, monitor->m_transformedSize.x,
...
static void init_functions() {
    ...
    static auto FNS_RT = HyprlandAPI::findFunctionsByName(
        PHANDLE,
        "_ZN6Render2GL15CHyprOpenGLImpl13renderTextureEN9Hyprutils6Memory14CSharedPointerINS_"
        "8ITextureEEERKNS2_4Math4CBoxENS1_18STextureRenderDataE");
    if (FNS_RT.empty())
    ...
    static auto FNS_RB = HyprlandAPI::findFunctionsByName(
        PHANDLE,
        "_ZN6Render2GL15CHyprOpenGLImpl12renderBorderERKN9Hyprutils4Math4CBoxERKN6Config18CGradient"
        "ValueDataENS1_17SBorderRenderDataE");
    if (FNS_RB.empty())
    ...
    static auto FNS_RB2 = HyprlandAPI::findFunctionsByName(
        PHANDLE,
        "_ZN6Render2GL15CHyprOpenGLImpl12renderBorderERKN9Hyprutils4Math4CBoxERKN6Config18CGradient"
        "ValueDataESA_fNS1_17SBorderRenderDataE");
    if (FNS_RB2.empty())
    ...
    static auto FNS_BO = HyprlandAPI::findFunctionsByName(
        PHANDLE,
        "_ZN6Render13IHyprRenderer29shouldUseNewBlurOptimizationsEN9Hyprutils6Memory14CSharedPointe"
        "rIN7Desktop4View13CLayerSurfaceEEENS3_INS5_7CWindowEEE");
    if (FNS_BO.empty())
    ...
    static auto FNS2 = HyprlandAPI::findFunctionsByName(
        PHANDLE,
        "_ZN6Render13IHyprRenderer18shouldRenderWindowEN9Hyprutils6Memory14CSharedPointerIN7Desktop"
        "4View7CWindowEEENS3_IN7Monitor8CMonitorEEE");
    if (FNS2.empty())
    ...
    static auto FNS3 = HyprlandAPI::findFunctionsByName(
        PHANDLE,
        "_ZN6Render13IHyprRenderer12renderWindowEN9Hyprutils6Memory14CSharedPointerIN7Desktop4View7"
        "CWindowEEENS3_IN7Monitor8CMonitorEEERKNSt6chrono10time_pointINSB_3_V212steady_clockENSB_"
        "8durationIlSt5ratioILl1ELl1000000000EEEEEEbNS_15eRenderPassModeEbb");
    if (FNS3.empty())
...
static void register_callbacks() {
    ...
    callbackListeners.push_back(Event::bus()->m_events.input.mouse.button.listen(on_mouse_button));
    callbackListeners.push_back(Event::bus()->m_events.input.mouse.move.listen(on_mouse_move));
    callbackListeners.push_back(Event::bus()->m_events.input.mouse.axis.listen(on_mouse_axis));
    callbackListeners.push_back(Event::bus()->m_events.input.keyboard.key.listen(on_key));

    // TODO: support touch
    callbackListeners.push_back(Event::bus()->m_events.input.touch.down.listen(
        [](const ITouch::SDownEvent&, Event::SCallbackInfo& info) { cancel_event(info); }));
    callbackListeners.push_back(Event::bus()->m_events.input.touch.up.listen(
        [](const ITouch::SUpEvent&, Event::SCallbackInfo& info) { cancel_event(info); }));
    callbackListeners.push_back(Event::bus()->m_events.input.touch.motion.listen(
        [](const ITouch::SMotionEvent&, Event::SCallbackInfo& info) { cancel_event(info); }));

    callbackListeners.push_back(Event::bus()->m_events.gesture.swipe.begin.listen(on_swipe_begin));
    callbackListeners.push_back(
        Event::bus()->m_events.gesture.swipe.update.listen(on_swipe_update));
    callbackListeners.push_back(Event::bus()->m_events.gesture.swipe.end.listen(on_swipe_end));

    callbackListeners.push_back(Event::bus()->m_events.config.reloaded.listen(on_config_reloaded));
    callbackListeners.push_back(Event::bus()->m_events.monitor.added.listen(register_monitors));
    callbackListeners.push_back(Event::bus()->m_events.monitor.removed.listen(on_monitor_removed));
}
...
#define addConfigValue(T, config, descr, value)                                                    \
    do {                                                                                           \
        SP<Config::Values::IValue> ivalue =                                                        \
            makeShared<T>("plugin:hyprtasking:" config, (descr), (value));                         \
        const auto RET = Config::mgr()->registerPluginValue(PHANDLE, ivalue);                      \
        if (!RET) {                                                                                \
            Log::logger->log(ERR, "[Hyprtasking] could not register value \"{}\"",                 \
                             ivalue->name());                                                      \
        }                                                                                          \
    } while (0)
...
void shutdownWorkspaceOverview() {
    ...
    ht_manager->hide_all_views_immediately();
    ht_manager->reset();
    ht_manager.reset();
    overview_scaled_render = false;
    overview_scaled_clip.reset();
    overview_scaled_alpha = 1.F;
}

bool workspaceOverviewActive() {
    return ht_manager != nullptr && ht_manager->has_active_view();
}
