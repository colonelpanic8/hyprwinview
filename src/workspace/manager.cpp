...
PHLWINDOW HTManager::get_window_from_cursor(bool return_focused) {
    ...
    if (!cursor_view->active || !cursor_view->layout->should_manage_mouse()) {
        return Desktop::viewState()->hitTest().windowAt(mouse_coords,
                                                        Desktop::View::RESERVED_EXTENTS |
    ...
    const Vector2D ws_coords =
        cursor_view->layout->global_to_local_ws_unscaled(mouse_coords, ws_id) +
...
void HTManager::hide_all_views_immediately() {
    swipe_state = HT_SWIPE_NONE;
    swipe_amt   = 0.0;
    ...
    for (PHTVIEW view : views) {
        if (view != nullptr && (view->active || view->closing || view->navigating))
...
void HTManager::refresh_all_grid_caches() {
    ...
    std::sort(grids.begin(), grids.end(),
              [](HTLayoutGrid* a, HTLayoutGrid* b) { return a->get_view_id() < b->get_view_id(); });
...
void HTManager::remove_view_for_monitor_id(MONITORID mid) {
    std::erase_if(views, [mid](const PHTVIEW& v) { return v == nullptr || v->monitor_id == mid; });
}
