...
bool HTLayoutBase::should_render_window(PHLWINDOW window) {
    ...
    return ((should_render_window_t)(should_render_window_hook->m_original))(g_pHyprRenderer.get(),
                                                                             window, monitor);
}
...
void HTLayoutBase::render_jump_labels() {
    ...
    const CHyprColor      label_color{static_cast<uint64_t>(label_color_value)};
    const CHyprColor      background_color{
        static_cast<uint64_t>(HTConfig::value<Config::INTEGER>("jump:label_background"))};
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        const std::string texture_key = std::string(1, LABELS[i]) + ":" +
            std::to_string(font_size) + ":" + std::to_string(label_color_value);
        auto& texture = texture_cache[texture_key];
        if (texture == nullptr) {
            texture = g_pHyprRenderer->renderText(std::string(1, LABELS[i]), label_color, font_size,
                                                  false, "", 0, 700);
        }
        if (texture == nullptr)
            continue;
...
void              HTLayoutBase::post_render() {
    ...
    std::erase_if(g_pHyprRenderer->m_renderPass.m_passElements, [&first](const auto& e) {
        const auto* data = [&]() {
            if constexpr (requires { e.element; })
                return &e;
            else
                return e.get();
        }();
        bool res = data->element->passName() == CLEAR_PASS_ELEMENT_NAME && !first;
        first    = false;
        return res;
    });
    render_jump_labels();
...
CBox HTLayoutBase::get_global_window_box(PHLWINDOW window, WORKSPACEID workspace_id) {
    ...
    const Vector2D bottom_right = local_ws_unscaled_to_global(
        ws_window_box.pos() + ws_window_box.size() - monitor->m_position, workspace->m_id);
