...
HTLayoutLinear::HTLayoutLinear(VIEWID new_view_id) : HTLayoutBase(new_view_id) {
    ...
    Animation::mgr()->createAnimation(
        0.f, scroll_offset, anim_tree->getAnimationPropertyConfig("windowsMove"), AVARDAMAGE_NONE);
    Animation::mgr()->createAnimation(
        0.f, view_offset, anim_tree->getAnimationPropertyConfig("windowsMove"), AVARDAMAGE_NONE);
    Animation::mgr()->createAnimation(
        0.f, blur_strength, anim_tree->getAnimationPropertyConfig("fadeIn"), AVARDAMAGE_NONE);
    Animation::mgr()->createAnimation(
        0.f, dim_opacity, anim_tree->getAnimationPropertyConfig("fadeDim"), AVARDAMAGE_NONE);
...
CBox HTLayoutLinear::calculate_ws_box(int x, int y, HTViewStage stage) {
    ...
    const float ws_y =
        calculate_y(monitor->m_transformedSize.y, use_view_offset, HEIGHT) + GAP_SIZE;
...
void HTLayoutLinear::build_overview_layout(HTViewStage stage) {
    ...
    for (const auto& [x, ws_id] : monitor_workspaces | std::views::enumerate) {
        ...
        overview_layout[ws_id] = {static_cast<int>(x), 0, ws_box};
    }
}
...
void HTLayoutLinear::render() {
    ...
    Animation::Workspace::startAnimation(start_workspace, Animation::Workspace::ANIMATION_TYPE_OUT,
                                         false, true);
    ...
    Animation::Workspace::startAnimation(start_workspace, Animation::Workspace::ANIMATION_TYPE_IN,
                                         false, true);
    ...
    CBox view_box = {{0.f, calculate_y(monitor->m_transformedSize.y, view_offset->value(), HEIGHT)},
                     {(float)monitor->m_transformedSize.x, (float)HEIGHT}};

    CRectPassElement::SRectData data;
    data.color =
        CHyprColor{static_cast<uint64_t>(HTConfig::value<Config::INTEGER>("bg_color"))}.stripA();
    data.box = view_box;
    ...
    for (const auto& [ws_id, ws_layout] : overview_layout) {
        ...
        const Config::CGradientValueData border_col =
            workspace == big_ws ? *ACTIVECOL : *INACTIVECOL;
